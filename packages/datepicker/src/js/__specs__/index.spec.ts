...
describe('#forceValidDay', () => {
  ...
  test('below = 00 july 1964', () => {
    expect(subject.forceValidDay({ mm: '07', dd: '00', yyyy: '1964' })).toBe(
      '1'
    )
  })

  test('above = 31 nov 1964', () => {
    expect(subject.forceValidDay({ mm: '11', dd: '31', yyyy: '1964' })).toBe(
      '30'
    )
  })

  test('matches = 29 feb 1964 had 29 days', () => {
    expect(subject.forceValidDay({ mm: '02', dd: '29', yyyy: '1964' })).toBe(
      '29'
    )
  })
})
...
describe('#getFirstDayOfWeekForMonth', () => {
  test('0-based Tuesday returned', () => {
    expect(subject.getFirstDayOfWeekForMonth({ mm: '4', yyyy: '2008' })).toBe(
      '2'
    )
  })

  test('0-based Sunday returned', () => {
    expect(subject.getFirstDayOfWeekForMonth({ mm: '2', yyyy: '2009' })).toBe(
      '0'
    )
  })
})
...
describe('#formatDate', () => {
  test('no value', () => {
    expect(subject.formatDate()).toEqual()
  })
