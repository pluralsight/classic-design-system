...
exports.generateComponents = async ({ src, dest }) => {
  const ext = 'dist.tsx'
  try {
    ...
    const names = await Promise.all(
      files.map(async file => {
        ...
        const fileContents = generateComponent(name, svgWithReactAttrs)
...
const generateComponent = (componentName, svgString) => {
  const regex = />/

  return `
import React from 'react'
import { forwardRefWithStatics } from '../../../util'
import { Icon, IconProps, IconStatics } from '..'

export const ${componentName} = forwardRefWithStatics<IconProps,
HTMLDivElement,
IconStatics
>((props, ref) => {
