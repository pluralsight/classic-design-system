...
import Icon from ${core ? "'../index.js'" : "'@pluralsight/ps-design-system-icon'"}
export const ${componentName} =  React.forwardRef(function ${componentName}({'aria-label': ariaLabel, ...props}, ref){ return (
  <Icon {...props} ref={ref}>
    ${svg.replace(regex, "role='img' {...(ariaLabel && { 'aria-label': ariaLabel })}>")}
  </Icon>
)})
...
const exportTemplate = filePaths => [...filePaths]
...
exports.buildSvgIcon = async ({src, dest, ext = 'dist.js', core}) => {
  try {
    ...
    const filePaths = await Promise.all(files.map(async file => {
      const componentName = `${dashToPascalCase(path.basename(file, '.svg').split('.')[0])}Icon`
      const svg = await fs.readFile(file, 'utf8')
      const { data } = await svgo.optimize(svg)
      const component = componentTemplate(data, componentName, core)
