...
import IconCommonSet from './icon-common-set.tsx'
...
Icons can be used at 3 standard sizes: Small (16px), Medium (24px), Large (48px)
...
An Icon will inherit it's color from the font color of it's parent elements in the browser. When using Icon with Text place inside the Text component if you wish for it to inherit color styles. You can override the color by styling the icon with the desired `color` in css or inline styles as shown below.

```typescript
import React from 'react'
import { PathIcon } from '@pluralsight/ps-design-system-icon'

const Comp = () => (
  <div className="example-flex-colum">
    <div style={{ color: '#0084BD' }}>
...
Additionally the `color` prop makes available the following colors.

```typescript
import React from 'react'
import { StarIcon } from '@pluralsight/ps-design-system-icon'

const Comp = () => (
  <div className="example-flex-row">
    <StarIcon color={StarIcon.colors.textIconHighOnDark} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.textIconLowOnDark} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.textIconHighOnLight} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.textIconLowOnLight} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.red} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.blue} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.green} size={StarIcon.sizes.large} />
    <StarIcon color={StarIcon.colors.yellow} size={StarIcon.sizes.large} />
  </div>
)
...
Current icon set. [Submit new icons through Github](https://github.com/pluralsight/design-system/issues).
...
<Usage install="npm install @pluralsight/ps-design-system-icon"
  import="import Icon from '@pluralsight/ps-design-system-icon'"
  packageName="icon"
/>

<Types.Table>
  <Types.Prop name="color" type={<Types.Enum enum={Icon.colors}/>} desc="color presets" default="currentColor"/>
  <Types.Prop name="size" type={<Types.Enum enum={Icon.sizes}/>} desc="icon size (from *Icon.sizes)"/>
</Types.Table>

<h2 id="common-icon-set">Common icon set</h2>
