...
## Examples: controlled
Use when there is a need to modify open state or hook into onToggle callback.
The Drawer only takes two props to take control of toggle microinteraction, (**onToggle**, **isOpen**).


### Mount drawer open (AKA startOpen)
...
### Prevent clicking on `<Drawer.Summary />` from opening `<Drawer />`
