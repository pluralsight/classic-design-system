---
name: Tag
route: /components/tag
---

import Tag from '@pluralsight/ps-design-system-tag'

# Tag

<TableOfContents {...props} />

<Intro>Tags allow users to categorize content. They can represent keywords or people.</Intro>

<Usage
  figmaUrl="https://www.figma.com/file/YsTklBecdddy9RZ3HXddIseD/?node-id=10232%3A1"
  install="npm install @pluralsight/ps-design-system-tag"
  import="import Tag from '@pluralsight/ps-design-system-tag'"
  packageName="tag"
  version={props.version}
/>

## Examples

### Size

Tags come in two sizes. Try to use the default size, `medium`, whenever possible.

```typescript
import Tag from '@pluralsight/ps-design-system-tag'
import React from 'react'

const Example: React.FC = props => {
  return (
    <div className="example-grid--col-2">
      <Tag size={Tag.sizes.small}>small</Tag>
      <Tag size={Tag.sizes.medium}>medium</Tag>
    </div>
  )
}

export default Example
```

### Icon

A tag may contain an icon, consistently placed on the right. An icon may also
receive on `onClick` prop in this position.

```typescript
import { XIcon } from '@pluralsight/ps-design-system-icon'
import Tag from '@pluralsight/ps-design-system-tag'
import React from 'react'

const Example: React.FC = props => {
  return (
    <div className="example-grid--col-3">
      <Tag icon={<XIcon />}>With Icon</Tag>

      <Tag icon={<XIcon />} size={Tag.sizes.small}>
        With Icon
      </Tag>

      <Tag
        icon={<XIcon onClick={_ => alert('icon clicked')} />}
        size={Tag.sizes.small}
      >
        With Icon
      </Tag>
    </div>
  )
}

export default Example
```

### Hover state

A tag may made actionable by adding an `onClick` or `href`. If it's actionable,
a standard hover state is present.

```typescript
import Tag from '@pluralsight/ps-design-system-tag'
import React from 'react'

const Example: React.FC = props => {
  return (
    <div className="example-grid--col-2">
      <Tag onClick={_ => alert('click')}>With Click</Tag>
      <Tag href="https://duckduckgo.com">With Link</Tag>
    </div>
  )
}

export default Example
```

### As a Toggle

A tag is sometimes used as a toggle. The toggled-on state is trigged with the
`isPressed` prop. It will handle styling and accessibility concerns. Controlling
applications are in charge of storing toggled state.

```typescript
import Tag from '@pluralsight/ps-design-system-tag'
import React from 'react'

const Example: React.FC = props => {
  return (
    <div className="example-grid--col-2">
      <Tag isPressed>Toggled on</Tag>
      <Tag>Toggled off</Tag>
    </div>
  )
}

export default Example
```

### Error

Error states are engaged with the `error` flag. This is useful when tags are
used from within a form

```typescript
import Tag from '@pluralsight/ps-design-system-tag'
import React from 'react'

const Example: React.FC = props => {
  return <Tag error>Problem tag</Tag>
}

export default Example
```

## Accessibility

**WCAG 2.1 AA Compliance**

<CheckCircleFilledIcon color="green" size="xSmall" /> 100% axe-core tests
<br />
<WarningFilledIcon color="red" size="xSmall" /> Manual audit

## Props

<TypesTable>
  <TypesProp
    name="error"
    type="boolean"
    desc="error state flag"
    default="false"
  />
  <TypesProp name="icon" type="*Icon" desc="Icon component, right-aligned" />
  <TypesProp
    name="isPressed"
    type="boolean"
    desc="for accessibility of tags used as toggles"
    default="false"
  />
  <TypesProp
    name="size"
    type={<TypesEnum enum={Tag.sizes} />}
    desc="tag size (from Tag.sizes)"
    default={Tag.sizes.medium}
  />
</TypesTable>
