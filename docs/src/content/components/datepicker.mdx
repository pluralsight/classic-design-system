...
<TypesTable>
  ...
  <TypesProp
    name="onSubBlur"
    type="(React.FocusEvent, Date) => void"
    desc="on subfield blur, returns undefined if invalid date"
  />
  <TypesProp
    name="onSelect"
    type="(React.FocusEvent | React.MouseEvent, Date) => void"
    desc="triggered when a valid date selected from calendar or after sub field blur"
  />
  <TypesProp
    name="subLabel"
    type="string"
