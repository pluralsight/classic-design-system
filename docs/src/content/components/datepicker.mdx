...
<Intro>
  DatePicker allows us to build date picker components of varying patterns using
  the `useDayzed` hook from{' '}
  <a href="https://github.com/deseretdigital/dayzed" target="_blank">
    dayzed
  </a>
  . These components allow us to make single date, multi-date or date range selectors.
</Intro>

<Usage
  figmaUrl="https://www.figma.com/file/YsTklBecdddy9RZ3HXddIseD/?node-id=10235%3A29839"
...
<TypesTable defaultColumn={false}>
  ...
  />
  <TypesProp name="value" type="Date" desc="date to set picker to" />
</TypesTable>
...
return (
  <Calendar {...dayzedData}>/** CalendarDates or something custom **/</Calendar>
)
```
...
```typescript noRender
import { useDayzed } from 'dayzed'
import { Calendar, CalendarDates, useIsInRange } from '@pluralsight/ps-design-system-datepicker'
...
<TypesTable defaultColumn={false}>
  ...
  />
  <TypesProp name="selected" required type="Date[]" desc="the selected date" />
  <TypesProp
    name="setSelected"
    required
...
return (
  <>
    <TextInput
      ...
      value={value}
    />
    <Calendar {...dayzedData} slide={slide}>
      <CalendarDates getDateProps={getDateProps}>
        {renderProps => {
          return <button {...renderProps} />
        }}
...
1. Create an `onDateSelected` callback that will update the selected and open states when a calendar date is clicked along with any side effects desired.
1. Destructure `getDateProps` and pass it to the `CalendarDates` component which uses a renderProp pattern to apply styles and handlers to buttons that represent individual dates in a month.
1. Pass `selected`, `setSelected`, and `onDateSelected` to the `useDayzed` hook from the dayzed library along with any other [dayzed props](https://github.com/deseretdigital/dayzed#props) desired.
1. Create a handleIconClick callback to be passed to CalendarIcon and will toggle the open state when clicked.
1. Create a new state object & setter `open: boolean` and `setOpen: React.Dispatch<React.SetStateAction<ValueOf<typeof slides>>>` and pass slide to the Calendar.
1. Use the `useDateSelectChange` hook and pass it `selected`, `setSlide`, and `setSelected`. Destructure the returned value and onChange callback and pass them to the TextInput. To manipulate the calendar using an input will require a bit more setup. The DatePicker package provides us with a handy hook to reduce the amount of boilerplate necessary for such a composite component, `useDateSelectChange`.
