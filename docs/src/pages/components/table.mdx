      ...
      <Table.Row>
        {FIELDS.map(field => (
          <Table.ColumnHeader
            align={field.align}
            key={field.key}
            onClick={(_evt, dir) => { handleSortClick(field.key, dir) }}
      ...
      {sortedRows.map(row => (
        <Table.Row key={row.name}>
          <Table.Cell>
            ...
            />
            <div style={{ marginLeft: layout.spacingSmall }}>
              {row.name}
            </div>
          </Table.Cell>
...
<Types.Table>
  <Types.Prop
    name="align"
    type={<Types.Enum enum={Table.aligns} />}
    desc="text alignment"
...
<Types.Table>
  ...
  <Types.Prop
    name="sort"
    type={<Types.Enum enum={Table.sorts} />}
