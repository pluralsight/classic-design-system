...
<Types.Table>
  <Types.Prop name="active" type="boolean" desc="styled active (automatically set)" />
  <Types.Prop name="href" type="string" desc="url of resource (renders as anchor)" />
  <Types.Prop name="id" required type="string|number" desc="id tying list item to panel" />
  <Types.Prop name="onClick" required type="Event => void" desc="callback when list item clicked" />
</Types.Table>

### Tab.Panel

<Types.Table>
  <Types.Prop name="labelledBy" required type="string|number" desc="id tying panel to list item" />
</Types.Table>
