...
receiver.router.get('/oauth-callback', async (request, res) => {
  ...
  const insertQuery = {
    text: 'INSERT INTO authenticatedusers(slack_id, access_token) VALUES($1, $2) ON CONFLICT (slack_id) DO UPDATE SET access_token = $2',
  ...
  try {
    await app.client.chat.postMessage({
      token: process.env.SLACK_BOT_TOKEN,
      channel: userID,
      text: "You can now post issues to design-system under your GitHub account using the `/issue` command. \n\n Use the `issue-signout` command to sign-out."
    });
  }
  catch (error) {
    console.error(error);
